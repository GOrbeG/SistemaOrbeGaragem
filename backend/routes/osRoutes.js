...
router.post(
  ...
  [
    ...
    body('veiculo_id').isInt().withMessage('ID do veículo é obrigatório e deve ser numérico'),
    body('usuario_id').isInt().withMessage('ID do usuário é obrigatório e deve ser numérico'),
    body('status').notEmpty().withMessage('Status é obrigatório'),
    body('descricao').notEmpty().withMessage('Descrição é obrigatória'),
    body('valor_total').isFloat().withMessage('Valor total deve ser numérico'),
    body('data_agendada').optional().isISO8601().toDate().withMessage('Data agendada inválida')
  ],
  async (req, res) => {
    const erros = validationResult(req);
    if (!erros.isEmpty()) return res.status(400).json({ erros: erros.array() });

    const { cliente_id, veiculo_id, usuario_id, status, descricao, valor_total, data_agendada } = req.body;
    try {
      const result = await db.query(
        'INSERT INTO ordens_servico (cliente_id, veiculo_id, usuario_id, status, descricao, valor_total, data_agendada) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
      ...
      await registrarHistorico({
        ...
        entidade_id: novasOS.id,
        dados_novos: novaOS
      });

      //envio de e-mail
      if (status === 'agendada') {
      const clienteResult = await db.query('SELECT nome, email FROM clientes WHERE id = $1', [cliente_id]);
      const cliente = clienteResult.rows[0];

      if (cliente?.email) {
        const veiculoResult = await db.query('SELECT modelo FROM veiculos WHERE id = $1', [veiculo_id]);
        ...
        await enviarEmail(cliente.email, 'Agendamento Confirmado - Orbe Garage', mensagemHtml);
      }
    }
    ...
    await db.query(
      `INSERT INTO notificacoes (usuario_id, mensagem, tipo) VALUES ($1, $2, $3)`,
      [usuario_id, `Nova OS agendada para ${new Date(data_agendada).toLocaleDateString('pt-BR')}`, 'info']
    );
...
router.put(
  ...
  async (req, res) => {
    ...
    const { cliente_id, veiculo_id, usuario_id, status, descricao } = req.body;
    try {
      const osAntes = await db.query('SELECT * FROM ordens_servico WHERE id = $1', [req.params.id]);

    const result = await db.query(
    'UPDATE ordens_servico SET cliente_id = $1, veiculo_id = $2, usuario_id = $3, status = $4, descricao = $5, valor_total = $6, data_agendada = $7 WHERE id = $8 RETURNING *',
    [cliente_id, veiculo_id, usuario_id, status, descricao, valor_total, data_agendada, req.params.id]
    );
      if (result.rows.length === 0) return res.status(404).json({ error: 'Ordem de serviço não encontrada' });
      ...
      await registrarHistorico({
        ...
        dados_anteriores: osAntes.rows[0],
...
router.delete('/:id', checkPermissao(['administrador', 'funcionario']), async (req, res) => {
  try {
    const osAntes = await db.query('SELECT * FROM ordens_servico WHERE id = $1', [req.params.id]);
    const result = await db.query('DELETE FROM ordens_servico WHERE id = $1 RETURNING *', [req.params.id]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Ordem de serviço não encontrada' });
...
router.get('/:id/link', async (req, res) => {
  try {
    const token = jwt.sign({ os_id: req.params.id }, process.env.JWT_SECRET, { expiresIn: '12h' });
...
router.get('/visualizar/:token', async (req, res) => {
  try {
    ...
    const os = await db.query('SELECT * FROM ordens_servico WHERE id = $1', [decoded.os_id]);
    const itens = await db.query('SELECT * FROM itens_ordem WHERE ordem_id = $1', [decoded.os_id]);
...
router.get('/:id/exportar', async (req, res) => {
  try {
    const { id } = req.params;
    const os = await db.query('SELECT * FROM ordens_servico WHERE id = $1', [id]);
    const itens = await db.query('SELECT * FROM itens_ordem WHERE ordem_id = $1', [id]);

    if (os.rows.length === 0) return res.status(404).json({ error: 'OS não encontrada' });

    const doc = new PDFDocument();
    res.setHeader('Content-disposition', 'inline; filename="ordem_servico_' + id + '.pdf"');
    ...
    const total = itens.rows.reduce((acc, cur) => acc + parseFloat(cur.valor), 0);
    doc.moveDown();
